...
import { DocumentsStep } from "@/components/donor-wizard/steps/Documents";
...
function NewDonorWizard() {
  ...
  const create = useMutation({
    ...
    onSuccess: (donor) => {
      ...
      toast.success(`Donor ${donor.id} created`);
      navigate({ to: "/donors/$id", params: { id: donor.id } });
    },
    onError: (e) =>
      toast.error(e instanceof Error ? e.message : "Could not create donor"),
  });
  ...
  const handleCancel = () => {
    ...
    clear();
    // Reset state to a clean draft so the next visit starts fresh.
  ...
  const titles: Record<number, { title: string; description: string }> = {
    ...
    1: {
      ...
      description: "Attach the source PDFs now or skip and upload later.",
    },
    2: {
      title: "Review & create",
